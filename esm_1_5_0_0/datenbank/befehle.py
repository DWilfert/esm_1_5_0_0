...
# ==============================================================================
# INITIALISIERUNG DES SESSION STATE SPEICHERS
# ==============================================================================
...
class MockCursor:
    ...
    def _handle_select(self, sql, params):
        ...
        if "count(*)" in sql_lower:
            ...
            res = [{"count": len(store)}] if self.dictionary else [(len(store),)]
            self.results = res
            return
        ...
        if "distinct trim(vertragsart)" in sql_lower:
            va_list = sorted(list({str(x.get("vertragsart", "")).strip() for x in st.session_state.mock_vertragsanalyse if x.get("vertragsart")}))
            self.results = [{"vertragsart": v} for v in va_list] if self.dictionary else [(v,) for v in va_list]
            return
        ...
        if "from vertragsanalyse" in sql_lower and ("where id = %s" in sql_lower or "where v.id = %s" in sql_lower):
            target_id = params[0] if params else None
            matched = [dict(x) for x in st.session_state.mock_vertragsanalyse if str(x.get("id")) == str(target_id)]
            self.results = matched if self.dictionary else [list(x.values()) for x in matched]
            return

        if "from auffaelligkeiten" in sql_lower and "where anlage_id = %s" in sql_lower:
            target_id = params[0] if params else None
            matched = [dict(x) for x in st.session_state.mock_auffaelligkeiten if str(x.get("anlage_id")) == str(target_id)]
            self.results = matched if self.dictionary else [list(x.values()) for x in matched]
            return
    ...
    def _handle_insert(self, sql, params):
        ...
        tbl = m.group(1).lower() if m else ""
        store = getattr(st.session_state, f"mock_{tbl}", None)
        if store is not None and params:
            new_id = max([x.get("id", 0) for x in store] + [0]) + 1
            entry = {"id": new_id}
            store.append(entry)
            st.toast("✅ [Demo-Modus] Datensatz lokal erfolgreich gespeichert.", icon="ℹ️")
        self.rowcount = 1

    def _handle_update(self, sql, params):
        st.toast("✅ [Demo-Modus] Aktualisierung erfolgreich simuliert.", icon="ℹ️")
        self.rowcount = 1
    ...
    def _handle_delete(self, sql, params):
        ...
        if store is not None and params:
            target_id = params[0]
            setattr(st.session_state, f"mock_{tbl}", [x for x in store if str(x.get("id")) != str(target_id)])
            st.toast("🗑️ [Demo-Modus] Datensatz lokal entfernt.", icon="ℹ️")
        self.rowcount = 1

    def _handle_truncate(self, sql):
        m = re.search(r"table\s+[`]?([a-zA-Z0-9_]+)[`]?", sql, re.IGNORECASE)
        tbl = m.group(1).lower() if m else ""
        if hasattr(st.session_state, f"mock_{tbl}"):
            setattr(st.session_state, f"mock_{tbl}", [])
        self.rowcount = 1
...
# ==============================================================================
# HILFSFUNKTIONEN FÜR DATAFRAMES & READ_SQL
# ==============================================================================
...
def hole_wartungsvertraege_daten():
    _init_mock_store()
    df_v = pd.DataFrame(st.session_state.mock_vertragsanalyse)
    if not df_v.empty:
        firmen_map = {f["id"]: f["firmenname"] for f in st.session_state.mock_firmeninfo}
        anlagen_map = {a["id"]: a["anlagebezeichnung"] for a in st.session_state.mock_anlagen}
        if "firma_id" in df_v.columns:
            df_v["firma_name"] = df_v["firma_id"].map(firmen_map).fillna("Demo-Partner GmbH")
        if "anlage_id" in df_v.columns:
            df_v["anlage_name"] = df_v["anlage_id"].map(anlagen_map).fillna("Demo-Anlage")
    return df_v
...
def hole_wartungsuebersicht_daten():
    _init_mock_store()
    va = hole_wartungsvertraege_daten()
    if not va.empty:
        df_out = va.copy()
        if "vertragsname" in df_out.columns and "anlagenname" not in df_out.columns:
            df_out["anlagenname"] = df_out["anlagebezeichnung"]
        if "standort_text" in df_out.columns and "standort" not in df_out.columns:
            df_out["standort"] = df_out["standort_text"]
        if "zyklus_monate" in df_out.columns and "intervall_monate" not in df_out.columns:
            df_out["intervall_monate"] = df_out["zyklus_monate"]
        return df_out
    return pd.DataFrame()
