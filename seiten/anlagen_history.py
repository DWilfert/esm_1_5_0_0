                ...
                try:
                    query = """
                        SELECT 
                            ...
                            a.anlagebezeichnung AS 'Bezeichnung',
                            a.anlagetyp AS 'Typ',
                            a.hersteller AS 'Hersteller',
                            a.typ AS 'Modell',
                            a.seriennummer AS 'Seriennummer',
                            a.baujahr AS 'Baujahr',
                            s.ort AS 'Standort',
                            a.gebaudeteil AS 'Gebäudeteil',
                            a.etage AS 'Etage',
                            a.raumbezeichnung AS 'Raum',
                            k.kg_txt AS 'Kostengruppe',
                            u.unter_txt AS 'Untergewerk',
                            a.lebensdauer AS 'Lebensdauer',
                            a.zustand AS 'Zustand'
                        FROM anlagen a
                        LEFT JOIN standort s ON a.standort_id = s.id
                        LEFT JOIN kostengruppen k ON a.kostengruppe_nr = k.kg_nr
                        LEFT JOIN untergewerk u ON a.ugewerk_nr = u.unter_nr
                        WHERE a.id = %s
                    """
                    df_det = pd.read_sql(query, conn_det, params=(ausgewaehlte_id,))
                    if not df_det.empty:
                ...
                try:
                    query_v = """
                        SELECT 
                            ...
                            v.vertragsart AS 'Art',
                            f.firmenname AS 'Firma',
                            v.vertragsende AS 'Vertragsende',
                            v.kosten_bestand_pa AS 'Kosten p.a.',
                            v.naechste_wartung AS 'Nächste Wartung'
                        FROM vertragsanalyse v
                        LEFT JOIN firmeninfo f ON v.firma_id = f.id
                        WHERE v.anlage_id = %s
                    """
                    df_v = pd.read_sql(query_v, conn_v, params=(ausgewaehlte_id,))
                    if not df_v.empty:
        ...
        with t_historie:
            ...
            if conn_h:
                try:
                    query_h = """
                        SELECT 
                            au.vertrag AS 'Vertrag',
                            au.standort_text AS 'Standort',
                            au.kommentar AS 'Kommentar'
                        FROM auffaelligkeiten au
                        WHERE au.anlage_id = %s
                    """
                    df_h = pd.read_sql(query_h, conn_h, params=(ausgewaehlte_id,))
                    if not df_h.empty:
        ...
        with t_pruefungen:
            ...
            if conn_p:
                try:
                    query_p = """
                        SELECT 
                            s.anlagenklasse AS 'Anlagenklasse',
                            s.kurzfassung AS 'Kurzfassung',
                            s.intervall AS 'Intervall',
                            s.qualifikation AS 'Qualifikation',
                            s.wiederkehrende_pruefung AS 'Nächste Prüfung'
                        FROM service s
                        WHERE s.anlage_id = %s
                    """
                    df_p = pd.read_sql(query_p, conn_p, params=(ausgewaehlte_id,))
                    if not df_p.empty:
