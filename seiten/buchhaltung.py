...
def zeige_buchhaltung():
    ...
    if not df_finanzen.empty:
        if 'standort_text' not in df_finanzen.columns and 'standort' in df_finanzen.columns:
            df_finanzen['standort_text'] = df_finanzen['standort']
        if 'standort_text' not in df_finanzen.columns:
            df_finanzen['standort_text'] = 'Unbekannt'
        if 'kosten_bestand_pa' not in df_finanzen.columns:
            df_finanzen['kosten_bestand_pa'] = df_finanzen.get('volumen', 0).fillna(0)
        if 'benchmark_ais_pa' not in df_finanzen.columns:
            df_finanzen['benchmark_ais_pa'] = pd.to_numeric(df_finanzen.get('kosten_bestand_pa', 0), errors='coerce').fillna(0.0) * 0.82
        if 'naechste_wartung' not in df_finanzen.columns:
            df_finanzen['naechste_wartung'] = df_finanzen.get('laufzeit_bis', pd.Timestamp(datetime.now().date()).strftime('%Y-%m-%d'))
        if 'firma' not in df_finanzen.columns:
            df_finanzen['firma'] = df_finanzen.get('firmenname', df_finanzen.get('kunde', 'Unbekannt'))

        df_finanzen['kosten_bestand_pa'] = pd.to_numeric(df_finanzen['kosten_bestand_pa'], errors='coerce').fillna(0.0)
        df_finanzen['benchmark_ais_pa'] = pd.to_numeric(df_finanzen['benchmark_ais_pa'], errors='coerce').fillna(0.0)
        ...
        with t_bwa1:
            ...
            summary_df = std_gruppe.rename(columns={
            ...
            st.dataframe(prepare_display_dataframe(anzeige_bwa), use_container_width=True, hide_index=True)
        ...
        with t_bwa2:
            ...
            st.dataframe(prepare_display_dataframe(cash_df), use_container_width=True, hide_index=True)
        ...
        with t_bwa3:
            ...
            st.dataframe(prepare_display_dataframe(Journal_df), use_container_width=True, hide_index=True)
    else:
