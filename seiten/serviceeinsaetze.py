...
def zeige_serviceeinsaetze():
    ...
    if srv_aktion == TXT_SRV["act_hist"]:
        if df_service.empty:
            ...
            s_filter = st.radio(TXT_SRV["filter_lbl"], [TXT_SRV["opt_all"], "NP", "FG"], horizontal=True, key="srv_standort_filter_v7")
            ...
            if s_filter != TXT_SRV["opt_all"]:
                if "standort_text" in df_srv_f.columns:
                    df_srv_f = df_srv_f[df_srv_f["standort_text"] == s_filter]
                elif "standort" in df_srv_f.columns:
                    df_srv_f = df_srv_f[df_srv_f["standort"] == s_filter]
            ...
            if not df_srv_f.empty:
                ...
                record_mapping = {
                    r['id']: f"{r.get('anlagebezeichnung', 'Unbekannte Anlage')} ({r.get('standort_text', r.get('standort', 'NP'))}) - {str(r.get('kurzfassung', ''))[:30]}"
                ...
                if not s_standort or s_id is None: 
                    ...
                    if conn_ins:
                        ...
                        try:
                            ...
                            val = (s_id, s_standort, s_kl, s_kl_bez, s_anl_typ, s_k1, s_k2, s_kurz, s_int, s_hinw, s_gg, s_gt, s_qual, s_entl)
                            cur.execute(sql, val)
                            conn_ins.commit()
