...
from datenbank.befehle import hole_datenbank_verbindung, hole_wartungsuebersicht_daten
...
def zeige_wartungsanalyse():
    ...
    if lang == "de":
        TXT_WA = {
            "title": "Wartungsübersicht & Fristen-Monitoring (Demo-Modus)",
        ...
        TXT_WA = {
            "title": "Maintenance Overview & Deadline Monitoring (Demo)",
    ...
    if not df_wartung.empty:
        ...
        with t1:
            if not df_ueb.empty:
                ...
                for _, row in df_ueb.iterrows():
                    v_id = row["id"]
                    firma = row.get("firma", "Demo-Partner GmbH")
                    ...
                    with st.container(border=True):
                        ...
                        with col_btn:
                            if st.button("➕ 1W", key=f"btn_plus_1w_{v_id}", use_container_width=True, help=TXT_WA["btn_1w_help"]):
                                st.success(f"Vertrag '{vertrag}' {TXT_WA['succ_moved']} (Simuliert)")
                                st.rerun()
            else:
        ...
        with t2:
            if not df_ans.empty:
                ...
                anzeige_df = df_ans.drop(columns=["naechste_wartung", "id", "anlage_id"], errors="ignore").rename(columns={"anzeige_termin": "naechste_wartung"})
        ...
        with t3:
            if not df_ok.empty:
                ...
                anzeige_df = df_ok.drop(columns=["naechste_wartung", "id", "anlage_id"], errors="ignore").rename(columns={"anzeige_termin": "naechste_wartung"})
                st.dataframe(anzeige_df, use_container_width=True, hide_index=True)
            else:
